...
upload="${1:?Usage: kitmaker-release.sh <true|false> [dist-directory]}"
dist_dir="${2:-dist}"
...
payload="[]"
release_asset_base_url="https://github.com/${GITHUB_REPOSITORY}/releases/download/${GITHUB_REF_NAME}"

for wheel in "${wheels[@]}"; do
    wheel_url="${release_asset_base_url}/$(basename "${wheel}")"
    payload="$(
        jq \
