...
if [[ "${commit_on_release_branch}" == "true" ]]; then
    if [[ ! "${CI_COMMIT_TAG}" =~ ${stable_tag_pattern} ]]; then
        echo "A tag reachable from ${release_branch} must use the exact stable format vX.Y.Z." >&2
        exit 1
    fi
elif [[ ! "${CI_COMMIT_TAG}" =~ ${release_candidate_tag_pattern} ]]; then
...
echo "Validated release tag: ${CI_COMMIT_TAG}"
