...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_nautobot_nvdatamodels_xxx
namespace = Collection("nautobot_nvdatamodels")
namespace.configure(
    {
        "nautobot_nvdatamodels": {
            ...
            "project_name": "nautobot-nvdatamodels",
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_nvdatamodels.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_nvdatamodels.nautobot_ver,
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.nautobot_nvdatamodels.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_nvdatamodels.python_ver}...")
...
@task(
    help={
        "job_result_id": "Job result UUID to rerun",
        "local": "Whether to run the job on the local system and not on a worker (default: False)",
    }
)
...
@task(
    help={
        "name": "Name of the job to enable. If not provided, all jobs will be enabled.",
    },
    iterable=["name"],
)
def enable_jobs(context, name=None):
    """Enable jobs."""
...
def pylint(context):
    ...
    command = f"{base_pylint_command} nautobot_nvdatamodels"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("nautobot_nvdatamodels") / Path("migrations")
    if migrations_dir.is_dir():
        migrations_pylint_command = (
            ...
            " nautobot_nvdatamodels.migrations"
        )
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'nautobot_nvdatamodels/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_nvdatamodels.local):
    ...
    # Disabling for now as we don't yet have docs and it's breaking CI
...
@task
def publish(context):
    """Publish the app to the package index."""
